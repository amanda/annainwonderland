...
def list_frequent(people_list):
    ...
    sorted_count = sorted(
        people_count.keys(), key=people_count.get, reverse=True)
    return sorted_count
...
def get_cast_from_user():
    '''for easy user input'''
    players = raw_input(
        'enter the names of people you would like to insert into the text, separated by commas: ')
...
if __name__ == '__main__':
    ...
    if not args.list:
        ...
        with codecs.open(args.list, encoding='utf-8') as f:
            # hacky for list of hacker schoolers
            people_list = f.read().split(',')
